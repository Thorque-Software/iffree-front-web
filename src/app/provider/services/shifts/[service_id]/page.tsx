...
import { getShifts, getOneServiceDetailProvider } from '@/services/ApiHandler';
...
const ShiftsTable = ({ params }: ShiftTableProps) => {
  ...
  const fetchInitialData = async (providerId: string) => {
      setLoading(true);
      try {
        const res = await getOneServiceDetailProvider(providerId, service_id);
  ...
  useEffect(() => {
    if(!user) return;
    if (user.providerId) {
      fetchInitialData(user.providerId);
    }
  }, []);
